...
Public Structure InjectedInputTouchInfo
    ...
    ''' <summary>
    ''' 表示触控接触区域的边界框的屏幕坐标。以与设备无关的像素 (DIP) 为单位。 默认值为 0 × 0 矩形，以指针 PixelLocation 为中心。
    ''' </summary>
    Dim Contact As InjectedInputRectangle
    Dim ContactRaw As InjectedInputRectangle
    ''' <summary>
    ''' 指针设备绕长轴（z 轴，垂直于数字化仪的表面）的逆时针方向旋转角度。值介于 0 和 359 之间，其中 0 表示与 x 轴对齐并从左向右指向的触摸指针；增加值表示逆时针方向上的旋转度。 默认值为 0。
    ''' </summary>
    Dim Orientation As UInteger
    ''' <summary>
    ''' 由指针设备施加在数字化仪表面上的力。规范化为 0 和 1024 之间的范围。默认值为 0。
    ''' </summary>
